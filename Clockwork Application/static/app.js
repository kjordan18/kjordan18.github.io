...
xText
  .append("text")
  .attr("y", -26)
  .attr("data-name", "reference_time")
  .attr("data-axis", "x")
  .attr("class", "aText active x")
  .text("Reference_Time (Days since Oct 2015)");
// 2. seniority_id
xText
  .append("text")
  .attr("y", 0)
  .attr("data-name", "seniority_id")
  .attr("data-axis", "x")
  .attr("class", "aText inactive x")
  .text("Executive Seniority Title");
// 3. None
...
// 1.preferred_industry_id
yText
  .append("text")
  .attr("y", -26)
  .attr("data-name", "preferred_industry_id")
  .attr("data-axis", "y")
  .attr("class", "aText active y")
  .text("Industry Category ID");

// 2. Days to Close
yText
  .append("text")
  .attr("y", 26)
  .attr("data-name", "days_to_close")
  .attr("data-axis", "y")
  .attr("class", "aText inactive y")
  .text("Days to Close/Fill on Executive Search");
...
d3.csv("static/data_seniority.csv", function(data) {
  // Visualize the data
  console.log(data)
  visualize(data);
});
...
function visualize(theData) {
  ...
  var curX = "reference_time";
  var curY = "days_to_close";
  ...
  var toolTip = d3
    ...
    .html(function(d) {
      ...
      var theX;
      // Grab the state name.
      var theState = "<div>" + d.industry_name + "</div>";
      // Snatch the y value's key and value.
      var theY = "<div>" + curY + ": " + d[curY] + "%</div>";
      // If the x key is reference time
      if (curX === "reference_time") {
  ...
  theCircles
    ...
    .attr("class", function(d) {
      return "stateCircle " + d.seniority_name;
    })
    // Hover rules
  ...
  theCircles
    ...
    .on("mouseover", function(d) {
      ...
      toolTip.show(d);
      // Highlight the state circle's border
      d3.select("." + d.seniority_name).style("stroke", "#323232");
    })
    .on("mouseout", function(d) {
      // Remove tooltip
      toolTip.hide(d);
      // Remove highlight
      d3.select("." + d.seniority_name).style("stroke", "#e3e3e3");
    });
